...
hpc_pro <- hpc %>% 
  ...
  mutate(dateandtime= paste(Date, Time, sep=' '), DateSampled = as.Date(Date, format = '%d/%m/%Y'), TimeSampled = as.POSIXct(dateandtime, format = '%d/%m/%Y %H:%M:%S'), wkofdy = wday(DateSampled, label=TRUE)) 
hpc_pro <- transform(hpc_pro, wkofdy = factor(wkofdy))
head(hpc_pro)

## Plot 3
with(hpc_pro, plot(TimeSampled, Sub_metering_1, main ="",ylab="Energy sub metering", xlab = "", type = "n"))
with(hpc_pro, lines(TimeSampled,Sub_metering_1, type = "l", col = "black"))
with(hpc_pro, lines(TimeSampled,Sub_metering_2, type = "l", col = "red"))
with(hpc_pro, lines(TimeSampled,Sub_metering_3, type = "l", col = "blue"))
